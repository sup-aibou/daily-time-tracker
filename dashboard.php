...
if (strlen($_SESSION['user_id'] == 0)) {
	...
	<body>
		...
		<div class="col-sm-9 col-sm-offset-3 col-lg-10 col-lg-offset-2 main">
					...
					<div class="col-xs-6 col-md-3 col-lg-3 no-padding">
						<div class="panel panel-teal panel-widget border-right">
							<div class="row no-padding"><em class="fa fa-xl fa-car color-blue"></em>
								...
								<div class="text-muted">Total Customer Parked</div>
							</div>
						</div>
					</div>
				</div><!--/.row-->
			...
			$ret = mysqli_query($con, "SELECT count(ID) as id1 from vehicle_info where VehicleCategory='Motorcycle'");
			$row = mysqli_fetch_array($ret);

			$ret = mysqli_query($con, "SELECT count(ID) as id2 from vehicle_info where VehicleCategory='Car'");
			$row2 = mysqli_fetch_array($ret);

			$ret = mysqli_query($con, "SELECT count(ID) as id4 from vehicle_info where VehicleCategory='Truck'");
